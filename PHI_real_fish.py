...
class ayus(BaseModel):
    ...
    def __init__(self, data_num, hist, type):
        ...
        X, school = phi.data_pair_generate(data_num)
        time = np.arange(399, school.data.xy.shape[0])
    ...
    def pattern(self):
        MC1 = self.main_complex_dir[self.t//10 + 40]
        MC2 = self.main_complex_sp[self.t//10 + 40]
    ...
    def update(self):
        ...
        self.draw_text("PHI_dir:"+str(self.phi_dir[int(self.t/10)+ 40])[0:6], [400, 650], 12)
        self.draw_text("PHI_sp:"+str(self.phi_sp[int(self.t/10)+ 40])[0:6], [400, 620], 12)
...
if __name__ == '__main__':
    ...
    model = ayus(data_num=0, hist=200, type="core_dir")
