...
def data_unite(kind="PHI", s2="dshita",hist=400):
    # 同ディレクトリのファイルからファイル名を全て収得
    cwd = os.getcwd()
    #p_file = pathlib.Path(cwd)
    title = cwd + '/T_max_' + str(hist) + '/'
...
def data_for_IIT(index, hist):
    X_dir_phi = data_unite(kind="PHI", s2="dshita",hist=hist)
    data = data_unite(kind="Complex", s2="dshita",hist=hist)
    time = data[index].shape[0]
    main_comp_dir = transform(data, index, time)

    X_dir_sp = data_unite(kind="PHI", s2="speed",hist=hist)
    data = data_unite(kind="Complex", s2="speed",hist=hist)
    time = data[index].shape[0]
    main_comp_sp = transform(data, index, time)

    return X_dir_phi[index], main_comp_dir, X_dir_sp[index], main_comp_sp

def data_pair_generate(index, hist):
    school = gd.Schools(10, index, 1)
    [PHI_dir, Main_complex_dir, PHI_sp, Main_complex_sp] = data_for_IIT(index, hist)
